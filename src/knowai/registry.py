...
Stored at ~/.knowai/registry.toml so the CLI can find a workspace by name
no matter where its files live (a cloned knowledge repo, a custom path, or
the default ~/.knowai/workspaces/<name>/).

Schema (TOML):
    [workspaces]
    tutorial = "C:/Me/PersonalCoding/tutorial-knowai/tutorial-knowai-knowledge"
    other    = "/home/me/.knowai/workspaces/other"

Resolution order in paths.workspace_dir(name):
    1. registry entry (if present and the path still exists)
    2. ~/.knowai/workspaces/<name>/  (default)
"""
...
def _registry_path() -> Path:
    return knowai_home() / "registry.toml"
...
def _save_raw(entries: dict[str, str]) -> Path:
    ...
    lines = [
        "# Knowai workspace registry — maps workspace name to on-disk path.",
        "# Managed by `knowai init` and `knowai workspace register`.",
        "",
        "[workspaces]",
    ]
