...
Layout:
    ~/.knowai/workspaces/<workspace>/scans/<repo_name>.json

Purpose: dev clones only the repo they're working on. Knowai still
needs scan results for the other repos so AI can answer cross-repo
questions ("does the web repo have a useCheckout hook?").

Flow:
1. CI or tech lead runs `knowai workspace scan --persist` → writes
...
from knowai.paths import workspace_scans_dir
