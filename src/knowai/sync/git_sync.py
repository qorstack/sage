...
Wraps git commands so devs don't need to cd into ~/.knowai/workspaces/<name>/
...
class GitSync:
    ...
    def pull(self, auto_resolve: bool = True) -> tuple[bool, str]:
        ...
        if not self.is_git_repo():
            return False, "not a git repo — run `knowai sync init` first"
        ...
        if resolved_any:
            cm = _run(["commit", "--no-edit", "-m", "knowai: auto-merge memory/approvals"], self.path, check=False)
            if cm.returncode != 0:
    ...
    def push(self, message: str = "knowai: update knowledge", auto_commit: bool = True) -> tuple[bool, str]:
