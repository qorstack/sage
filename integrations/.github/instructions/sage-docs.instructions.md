...
- `api-flow` — frontend calling backend endpoints
...
SVG layout by doc type: api-flow → left-to-right swim lanes · backend-logic /
runbook → top-to-bottom nodes · architecture → left-to-right boxes ·
data-schema → grid.

**Diagram quality rules:**

- `sequenceDiagram` style: exact HTTP method + path on arrows, exact response
  `201 { field, field }`, every branch shown, DB queries shown
- `flowchart TD` style: trigger node with full request detail, decision diamonds
  with literal conditions, storage nodes naming table + operation, leaf nodes
  showing HTTP status + response shape
...
2. Extract CSS from the first ` ```css ` fenced code block
...
2. `<section class="diagram-section">` — inline SVG with zoom/pan (if applicable)
